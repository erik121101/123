...
def text_to_caption_data(text, audio_duration, words_per_line=6):
    ...
    return [
        {"start": i * time_per_line, "end": (i + 1) * time_per_line, "text": line}
        for i, line in enumerate(lines)
    ]


def abgr_to_rgb(abgr):
    """Convert ffmpeg ABGR hex (&H00FFFFFF) to #RRGGBB."""
    s = abgr.lstrip("&H").lstrip("0x").zfill(8)
    if len(s) == 8:
        return f"#{s[6:8]}{s[4:6]}{s[2:4]}"
    return "#ffffff"


def build_drawtext_filter(captions, fontsize, color_abgr, bold, position, bg_abgr):
    """
    Build a chained ffmpeg drawtext filter (no fontconfig needed).
    position: ASS alignment int (2=bottom-center, 8=top-center, 5=middle)
    """
    if not captions:
        return None

    fc = abgr_to_rgb(color_abgr) if color_abgr.startswith("&") else color_abgr

    # Parse background alpha from ABGR: &HAABBGGRR, AA is alpha (00=opaque, FF=transparent in ASS)
    # For drawtext boxcolor we convert to 0-1 where 0=transparent
    try:
        bg_aa = int(bg_abgr.lstrip("&H")[:2], 16)
        box_alpha = round(1.0 - bg_aa / 255, 2)  # ASS 00=opaque -> alpha 1.0
    except Exception:
        box_alpha = 0.5
    ...
    for cap in captions:
        txt = cap["text"] \
            ...
            .replace(":", "\\:") \
            .replace("%", "\\%")
        s, e = cap["start"], cap["end"]
        dt = (
            f"drawtext=text=\'{txt}\'"
            f":fontsize={fontsize}{bold_flag}"
            f":fontcolor={fc}"
            f":borderw=3:bordercolor=black@0.9"
            f":box=1:boxcolor=black@{box_alpha}:boxborderw=8"
...
def run_export(job_id, tts_text, settings):
    ...
    try:
        ...
        caption_data = text_to_caption_data(tts_text, tts_dur, words_per_line)
        ...
        fontsize = int(settings.get("cap_fontsize", 20))
        color    = settings.get("cap_color", "&H00FFFFFF")
        position = int(settings.get("cap_position", 2))
        bold     = bool(settings.get("cap_bold", True))
        cap_bg   = settings.get("cap_bg", "&H80000000")

        # Scale to 1080p vertical (Shorts: 1080x1920)
        scale_filter = "scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2"

        # Build drawtext filter (no fontconfig/subtitles filter needed)
        drawtext = build_drawtext_filter(caption_data, fontsize, color, bold, position, cap_bg)

        if drawtext:
            video_filter = f"[0:v]{scale_filter},{drawtext}[v]"
        else:
            video_filter = f"[0:v]{scale_filter}[v]"
        ...
        if r.returncode != 0:
            # fallback: scale only, no captions
            update_job(job_id, step="Caption error, export fara captions...", export_progress=70)
            r2 = subprocess.run(
                ["ffmpeg", "-y",
                 "-ss", str(trim_start),
                 "-stream_loop", "-1", "-i", str(muted_path),
