...
def export_video():
    ...
    caption_color   = data.get("caption_color", "&H00FFFFFF")
    caption_font    = data.get("caption_font", "Arial")
    caption_style   = data.get("caption_style", "outline")   # outline | box | shadow | none
    music_filename  = data.get("music_filename") or None
    music_start     = float(data.get("music_start", 0))
    music_volume    = float(data.get("music_volume", 0.08))
    auto_speed_tts  = bool(data.get("auto_speed_tts", False))
    ...
    threading.Thread(
        ...
        args=(export_job_id, job_id, trim_start, trim_end,
              captions_srt, caption_size, caption_color, caption_font,
              caption_style, music_filename, music_start, music_volume,
              auto_speed_tts),
        daemon=True
    ).start()
...
def run_export_pipeline(export_job_id, source_job_id, trim_start, trim_end,
    ...
    try:
        ...
        tts_working = trimmed_tts if trimmed_tts.exists() else tts_src

        # ── AUTO SPEED: stretch/shrink TTS to match video duration ────────
        if auto_speed_tts and duration and duration > 0:
            update_job(export_job_id, step="Se ajustează viteza vocii...", progress=55)
            tts_raw_dur = get_duration(tts_working)
            if tts_raw_dur > 0:
                speed_factor = tts_raw_dur / duration
                speed_factor = max(0.5, min(2.0, speed_factor))
                if abs(speed_factor - 1.0) > 0.02:
                    sped_tts = out_dir / "tts_sped.mp3"
                    # atempo supports 0.5–2.0; chain filters for extreme values
                    if speed_factor <= 2.0 and speed_factor >= 0.5:
                        atempo_filter = f"atempo={speed_factor:.4f}"
                    elif speed_factor > 2.0:
                        atempo_filter = f"atempo=2.0,atempo={speed_factor/2.0:.4f}"
                    else:
                        atempo_filter = f"atempo=0.5,atempo={speed_factor*2.0:.4f}"
                    result = subprocess.run(
                        ["ffmpeg", "-y", "-i", str(tts_working),
                    ...
                    if result.returncode == 0:
                        tts_working = sped_tts

        tts_final = tts_working
        ...
        # Build force_style based on caption_style
        safe_color = caption_color if caption_color.startswith("&H") else "&H00FFFFFF"
        if caption_style == "box":
            style_str = (f"FontName={caption_font},FontSize={caption_size},Bold=1,"
                         f"PrimaryColour={safe_color},OutlineColour=&H00000000,"
                         f"BackColour=&HAA000000,BorderStyle=4,Outline=0,Shadow=0,"
                         f"Alignment=2,MarginV=30")
        elif caption_style == "shadow":
            style_str = (f"FontName={caption_font},FontSize={caption_size},Bold=1,"
                         f"PrimaryColour={safe_color},OutlineColour=&H00000000,"
                         f"BackColour=&H00000000,Outline=0,Shadow=3,"
                         f"Alignment=2,MarginV=30")
        elif caption_style == "none":
            style_str = (f"FontName={caption_font},FontSize={caption_size},Bold=1,"
                         f"PrimaryColour={safe_color},OutlineColour=&H00000000,"
                         f"BackColour=&H00000000,Outline=0,Shadow=0,"
                         f"Alignment=2,MarginV=30")
        ...
        # Escape SRT path for ffmpeg subtitles filter (Linux paths: escape colons)
